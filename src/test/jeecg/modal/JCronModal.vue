<template>
  <a-modal
    ...
    <div class="card-container">
            ...
            <a-radio-group v-model="result.year.cronEvery">
              ...
              <a-row>
                <a-radio value="2">每隔
                  ...
                  年执行 从
                  <a-input-number size="small" v-model="result.year.incrementStart" :min="+moment().format('YYYY')" :max="2119"></a-input-number>
                  年开始
                </a-radio>
              </a-row>
              <a-row>
                ...
                <a-icon type="caret-down" slot="suffixIcon" />
                  <a-select-option v-for="(val,index) in Array(100)" :key="index" :value="moment().format('YYYY')+index">{{ moment().format('YYYY')+index }}</a-select-option>
                </a-select>
              </a-row>
              <a-row>
                <a-radio value="4">从
                  <a-input-number size="small" v-model="result.year.rangeStart" :min="+moment().format('YYYY')" :max="2119"></a-input-number>
                  到
                  <a-input-number size="small" v-model="result.year.rangeEnd" :min="+moment().format('YYYY')" :max="2119"></a-input-number>
                  年之间的每一年
                </a-radio>
              </a-row>
            </a-radio-group>
      ...
      <div class="run-time" v-if="isShowRunTime">
        <p v-if="runTimeList.length">最近{{runTimeList.length}}次运行时间</p>
        <div class="run-time-item" v-for="item in runTimeList" :key="item">
          <div class="run-time-item-icon"></div>
          <p>{{item}}</p>
        </div>
      </div>
    </div>
...
<script>
  import { offlineGetCronNextTime } from '@/api/api'
  import moment from 'moment';
  export default {
    name:'VueCron',
    props:['data','isShowRunTime'],
    data(){
      return {
        moment,
        currentCron: '',
        ...
        defaultValue: {
          ...
          year:{
            ...
            incrementStart:2017,
            incrementIncrement:1,
            rangeStart:moment().format('YYYY'),
            rangeEnd: moment().format('YYYY'),
            specificSpecific:[],
          },
          label:''
        }
    ...
    computed: {
      ...
      cron(){
        this.currentCron = `${this.secondsText||'*'} ${this.minutesText||'*'} ${this.hoursText||'*'} ${this.daysText||'*'} ${this.monthsText||'*'} ${this.weeksText||'?'} ${this.yearsText||'*'}`
    ...
    watch:{
      ...
      currentCron(newCron,oldCron){
        if(this.isShowRunTime){
          offlineGetCronNextTime({cron:this.currentCron}).then(res=>{
            console.log(res,'runtime res');
            if(res.success){
              this.runTimeList = res.result || []
            } else {
              this.runTimeList = []
            }
          }).catch(()=>{
    ...
    methods: {
      show(){
        this.visible = true;
        if(this.isShowRunTime){
          offlineGetCronNextTime({cron:this.cron}).then(res=>{
            console.log(res,'runtime res');
            if(res.success){
              this.runTimeList = res.result || []
            } else {
      ...
      secondsReverseExp(seconds) {
        ...
        switch (true) {
          ...
          case val.includes('/'):
            ...
            break;
          case (val.includes(',')|| Number(val) !== NaN):
            second.cronEvery = '3';
        ...
        if (!days.includes('?')) {
          switch (true) {
            ...
            case days.includes('/'):
              ...
              break;
            case (days.includes(',')||Number(days) !==NaN):
              day.cronEvery = '5';
          ...
          switch (true){
            case weeks.includes('/'):
              ...
              break;
            case (weeks.includes(',')|| Number(weeks) !== NaN):
              day.cronEvery = '4';
      ...
      monthsReverseExp(cron) {
        ...
        switch (true){
          ...
          case months.includes('/'):
            ...
            break;
          case (months.includes(',')|| Number(months) !== NaN):
            month.cronEvery = '3';
        ...
        if(years){
          switch (true){
            ...
            case years.includes('/'):
              ...
              break;
            case (years.includes(',')|| Number(years) !== NaN):
              year.cronEvery = '3';
