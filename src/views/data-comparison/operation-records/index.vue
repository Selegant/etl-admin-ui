  ...
  <div class="h-full overflow-hidden">
    <n-card ref="cardRef" :title="(route.meta.title as string)" class="h-full shadow-sm rounded-16px">
      ...
      >
        ...
        <!-- <n-form-item label="采集模式" path="isIncrement">
...
import { qualityCheckRecordList, stopOffCollectionRecord } from '@/service';
...
const actionslist = [
  {
    localIcon: 'ci-window-code-block',
    value: '运行参数',
    onClick: (row: any) => {
...
async function getData(arg?: number) {
  ...
  const { data } = await qualityCheckRecordList(params);
