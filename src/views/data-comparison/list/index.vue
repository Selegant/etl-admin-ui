  ...
  <div class="h-full overflow-hidden">
    <n-card ref="cardRef" class="h-full shadow-sm rounded-16px">
      ...
      >
        ...
        <n-form-item label="采集源头" path="originalDbKey">
          <n-select
        ...
        <n-form-item label="目标源头" path="targetDbKey">
          <n-select
      ...
      <n-space justify="space-between">
        <n-space>
          ...
          <n-button strong secondary type="primary" @click="download">导出运行列表</n-button>
...
import qs from 'qs';
import {
  getQualityCheckJobList,
  qualityCheckJobDelete,
  offCollectionExecuteOnce,
  onlineJob,
  offlineJob,
  qualityCheckJobBatchStartScheduler,
  qualityCheckJobBatchStopScheduler,
  downloadFile,
  getDblist,
  getQualityCheckJobAllCronDict,
  getProjectList,
  batchOnline,
  batchOffline
} from '@/service';
...
// import IncTimeModal from '../component/inc-time-modal.vue';
      ...
      render: (rowData: any) => {
        return h(
          NSwitch,
          {
            ...
            'on-update:value': async (value: any) => {
...
async function batchdelete(type: number, code?: string) {
  ...
  const { error } = await qualityCheckJobDelete(codes);
  if (!error) {
...
function operationRecordClick(row: any) {
  routerPush({
    name: 'data-comparison_operation-records',
    query: { ...row }
  });
}
...
async function startStop(value: any, code?: any) {
  const { error } =
    value === 1
      ? await qualityCheckJobBatchStartScheduler({ codes: code ? [code] : checkedRowKeysRef.value })
      : await qualityCheckJobBatchStopScheduler({ codes: code ? [code] : checkedRowKeysRef.value });
  if (!error) {
    window.$message?.success(`${code ? '' : '批量'}${value === 1 ? '开启' : '关闭'}成功`);
    if (!code) {
      checkedRowKeysRef.value = [];
    }
    getData();
...
async function getData(arg?: number) {
  ...
  const { data } = await getQualityCheckJobList(params);
  endLoading();
...
async function getCron() {
  const { data } = await getQualityCheckJobAllCronDict();
  if (data) {
...
async function batchSelect(key: string) {
  ...
  if (key === '2') {
    const params = checkedRowKeysRef.value.map(v => {
      const obj: any = dataList.value.find(q => q.code === v);
      return {
