  ...
  <a-modal
    ...
    <a-form @submit="handleSubmit" :form="form">
      <a-row class="form-row" :gutter="24">
        ...
        >
          <a-input disabled v-decorator="['datasource.id', {rules:[{required: true, message: '数据源ID'}]}]" />
        </a-form-item>
      </a-row>
      <a-row class="form-row" :gutter="24">
        <a-form-item
...
<script>
import { testDataSource, updateDataSource } from '@/api/datax'
export default {
  name: 'EditForm',
  data () {
    return {
  ...
  methods: {
    ...
    edit (record) {
      ...
      this.visible = true
      this.$nextTick(() => {
        setFieldsValue({ datasource: record })
      })
    },
    testDataSource () {
    ...
    handleSubmit () {
      ...
      validateFields((errors, values) => {
        if (!errors) {
          updateDataSource(values.datasource)
            .then(res => {
              if (res.code === 200) {
